...
parser.add_argument("--index_sim", "-i", help="index of simulations")
args = parser.parse_args()
if args.num_sim is None:
    sys.exit("\nNumber of simulations is not provided.\n")
else:
    num_sim = int(args.num_sim)

if args.index_sim is None:
    sys.exit("\nIndex of simulations is not provided.\n")
else:
    index_sim = int(args.index_sim)
...
np.savetxt('/gpfs/alpine/csc299/scratch/litan/MOCU/Byung-Jun/KuramotoModel/N9ForShare/results/paramNaturalFrequencies{}.txt'.format(index_sim), w, fmt='%.64e')
np.savetxt('/gpfs/alpine/csc299/scratch/litan/MOCU/Byung-Jun/KuramotoModel/N9ForShare/results/paramInitialUpper{}.txt'.format(index_sim), aInitialUpper, fmt='%.64e')
np.savetxt('/gpfs/alpine/csc299/scratch/litan/MOCU/Byung-Jun/KuramotoModel/N9ForShare/results/paramInitialLower{}.txt'.format(index_sim), aInitialLower, fmt='%.64e')

##while (numberOfSimulationsPerMethod > numberOfVaildSimulations):
randomState = np.random.RandomState(int(index_sim))
a = np.zeros((N,N))
# for i in range(N):
#     for j in range(i+1,N):
...
np.savetxt('/gpfs/alpine/csc299/scratch/litan/MOCU/Byung-Jun/KuramotoModel/N9ForShare/results/paramCouplingStrength' + str(index_sim) + '.txt', a, fmt='%.64e')
test = np.loadtxt('/gpfs/alpine/csc299/scratch/litan/MOCU/Byung-Jun/KuramotoModel/N9ForShare/results/paramCouplingStrength' + str(index_sim) + '.txt')
for indexMethod in range(len(listMethods)):     
    ...
    print("Round: ", index_sim+1, "/", num_sim, "-", listMethods[indexMethod], "Iteration: ", index_sim, " Initial MOCU: ", MOCUInitial, " Computation time: ", time.time() - timeMOCU)
    ...
    outMOCUFile = open('/gpfs/alpine/csc299/scratch/litan/MOCU/Byung-Jun/KuramotoModel/N9ForShare/results/' + listMethods[indexMethod] + '_MOCU{}.txt'.format(index_sim), 'a')
    outTimeFile = open('/gpfs/alpine/csc299/scratch/litan/MOCU/Byung-Jun/KuramotoModel/N9ForShare/results/' + listMethods[indexMethod] + '_timeComplexity{}.txt'.format(index_sim), 'a')
    outSequenceFile = open('/gpfs/alpine/csc299/scratch/litan/MOCU/Byung-Jun/KuramotoModel/N9ForShare/results/' + listMethods[indexMethod] + '_sequence{}.txt'.format(index_sim), 'a')
